...
class RegisterLifecycleExtensions {
  ...
  with (classes) {
    [].concat(...classes)
      .forEach(clazz => {
        ...
        throw new Error(`Cannot load middleware ${clazz}. Only classes are supported, received ${typeof clazz}.`)
      })
  }
  ...
  implementedLifecycleMethods (classInstance) {
    // let baseClass = classInstance.constructor
  ...
  /**
   * Returns the list of inherited or implemented methods.
   * This list includes all methods, also the constructor.
   *
  ...
  classMethods (instance) {
    ...
    do {
      Object
        .getOwnPropertyNames(currentInstance)
        .map(item => properties.add(item))
    } while (
      (currentInstance = Object.getPrototypeOf(currentInstance))
    )

    return [...properties.keys()].filter(item => typeof instance[item] === 'function')
  }
