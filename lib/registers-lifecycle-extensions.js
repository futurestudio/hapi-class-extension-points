...
class RegisterLifecycleExtensions {
  ...
  from (clazz) {
    if (Typical.isClass(clazz)) {
      return this.loadFromClass(clazz)
    }
   ...
   * @throws
   */
  loadFromClass (Extension) {
    const middleware = new Extension(this.server)

    if (!this.hasLifecycleMethods(middleware)) {
    ...
    this.implementedLifecycleMethods(middleware).forEach(method => {
